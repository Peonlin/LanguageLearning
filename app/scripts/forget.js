$(document).ready(function() {
  ...
  $('.part2 .btn').click(function(){
    if($('input[name=valid]').val() == $('input[name=valid]').attr('valid')){
        showPart($('.part3'));
    }
    else{
      showMessage($('.part2'), '验证码有误！');
    }
  });
  $('.part3 .btn').click(function(){
    if($('input[name=valid]').val() == $('input[name=valid]').attr('valid') && $('input[name=password]').val() != ''){
      if($('input[name=password_confirm]').val() != $('input[name=password]').val())
        showMessage($('part3'), '两次输入密码不一致！');
      else
        sendChangePassword();
    }
  });
  function sendEmail(){
    ...
    .done(function(result) {
      if(result.message == 'success'){
        ...
        showMessage($('part1'),JSON.stringify(result));
      }
    })
  ...
  function showMessage(ob,str){
    ob.find('.message').text(str);
    ob.find('.message').show();
  }
  function showPart(ob){
    $('.part').hide();
    ob.show();
    $('.nav').eq(ob.index()).removeClass('disable');
  }
});
